...
- (void) fillInBlocksToUnrestrictedRow:(int)endRow {
    ...
    for (int section=(int)self.lastIndexPathPlaced.section; section<numSections; section++) {
        int numRows = (int)[self.collectionView numberOfItemsInSection:section];
        
        for (int row=(int)(!self.lastIndexPathPlaced? 0 : self.lastIndexPathPlaced.row+1); row<numRows; row++) {
